...
class League_model extends CI_Model {
    ...
    public function checkname($username='') {
      $sql = 'select * from club_user_info where user_name = '.$this->db->escape($username);
    ...
    public function signup($username='', $password='', $email='', $invite_code='', $introduction='', $contact='') {
      ...
      $res = $this->db->query($sql);
      $headicon = '/hehe.jpg';
      $sql = 'insert into club_user_info values('.$this->db->escape($uid).','.$this->db->escape($username).','.$this->db->escape($introduction).','
      .$this->db->escape($email).','.$this->db->escape($contact).','.$this->db->escape(0).','.$this->db->escape($headicon).','
      .$this->db->escape(date('Y-m-d H:i:s', time())).')';
      $res = $this->db->query($sql);
      // 使邀请码失效
    ...
    public function leagueinfo() {
      ...
      $sql = 'select * from club_user_info where user_id = '.$this->db->escape($uid);
      $res = $this->db->query($sql);
      if ($res->num_rows() === 0) {
        return array('flag' => -2);
      }
